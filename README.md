...
Envia una unica solicitud de aclaracion por WhatsApp al conductor Gammavet sin modificar la ruta
...
**Note:** The `_send_response()` helper in `function_logic.py` already implements this pattern correctly. You typically don't need to modify it.{% if cookiecutter.enable_dynamic_tools %}

### Dynamic Tools Preflight

This function declares `dynamic_tools: true` in `manifest.yml`. That means it can expose dynamic context to the agent wrapper through a preflight call before the normal function request path runs.

`src/handler.py` dispatches `event_type == "preflight_discover"` to `FunctionBackend.handle_preflight()` before `process_request()` is called. Implement `handle_preflight()` in `src/backend/function_logic.py`:

- `discover` mode reads `self.orchestration_event.extra_params.get("preflight_mode", "discover")` and returns a list of agent-facing tool definitions.
- `execute` mode performs the selected dynamic action. The scaffold raises `NotImplementedError` until you implement it.

Every preflight invocation emits one structured JSON log line. Keep these fields stable so CloudWatch Insights queries can isolate failing runtime-execution Lambdas:

| Field | Meaning |
|-------|---------|
| `preflight_mode` | Requested mode: `discover` or `execute` |
| `preflight_duration_ms` | Integer elapsed time for the preflight handler |
| `preflight_error` | Error string on failure, otherwise `null` |
| `function_uuid` | Lambda UUID from `FUNCTION_UUID` |
| `slug` | Tenant or organization slug associated with the request |
| `branch` | Tenant branch associated with the request |
| `action` | Dynamic action being executed, when present |

Use `logger.info(json.dumps({...}))` on success and `logger.error(json.dumps({...}), exc_info=True)` on failure.
{% endif %}
...
- The function name matches "PedirAclaracionFn"
...
- AWS CloudWatch Logs: `/aws/lambda/PedirAclaracionFn`
- Chask dashboard: Organization → Functions → PedirAclaracionFn
